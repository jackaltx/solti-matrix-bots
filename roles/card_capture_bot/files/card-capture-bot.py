...
Env vars controlling Vault paths (set by systemd template):
    ...
    VAULT_BOT_NAME    — Vault runtime path key for this bot (default: card-capture-bot)
...
def load_vault_credentials(homeserver_url: str, bot_user_id: str) -> None:
    ...
    bot_name    = os.getenv('VAULT_BOT_NAME', 'card-capture-bot')
    ...
    # Matrix — infrastructure secret; synapse path uses the Matrix localpart (@card-capture:domain)
    matrix_localpart = bot_user_id.lstrip('@').split(':')[0] if bot_user_id else 'card-capture'
