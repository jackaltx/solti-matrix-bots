...
Vault paths (KV v2, all under kv/data/):
    runtime/<bot_host>/card-capture/mongodb         → uri           (generated by mongodb configure)
    runtime/<bot_host>/card-capture/rustfs          → access_key, secret_key  (generated by rustfs configure)
    infrastructure/mylab/anthropic                  → api_key       (imported from LabSecrets.yml)
    infrastructure/<matrix_host>/synapse/bots/card-capture → password (imported from LabSecrets.yml)
...
def load_vault_credentials(homeserver_url: str, bot_user_id: str) -> None:
    ...
    mongo = _read(f"runtime/{bot_host}/card-capture/mongodb")
    ...
    s3 = _read(f"runtime/{bot_host}/card-capture/rustfs")
    ...
    # Matrix — infrastructure secret (imported from LabSecrets.yml, login for fresh token)
    matrix = _read(f"infrastructure/{matrix_host}/synapse/bots/card-capture")
